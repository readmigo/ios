...
/// ViewModel for TXT Reader
class TxtReaderViewModel: ObservableObject {
    // MARK: - Published Properties

    @Published var document: ParsedTxtDocument?
    ...
    func loadDocument(from url: URL) {
        ...
        Task { @MainActor in
            do {
                let doc = try await TxtParser.parse(url: url)
                self.document = doc
    ...
    func loadDocument(from data: Data, filename: String? = nil) {
        isLoading = true
        error = nil

        Task { @MainActor in
            if let doc = TxtParser.parse(data: data, filename: filename) {
                self.document = doc
                self.isLoading = false
            } else {
                self.error = "Failed to parse document"
                self.isLoading = false
            }
        }
    }
    ...
    func loadDocument(content: String, filename: String? = nil) {
        isLoading = true
        error = nil

        Task { @MainActor in
            let doc = TxtParser.parse(content: content, filename: filename)
            self.document = doc
            self.isLoading = false
        }
    }
    ...
    func getTextForTTS() -> String? {
        return currentChapter?.paragraphs.joined(separator: "\n\n")
    }
}
