...
struct CivilizationMapSection: View {
    ...
    /// 计算影响网络总数据量
    private var influenceNetworkDataCount: Int {
        let influences = civilizationMap.influences
        return influences.predecessors.count +
               influences.successors.count +
               influences.contemporaries.count +
               (influences.mentors?.count ?? 0) +
               (influences.students?.count ?? 0)
    }

    var body: some View {
        VStack(spacing: 16) {
            // Influence Network - 只有超过1条数据时才显示
            if influenceNetworkDataCount > 1 {
