...
struct DiscoverView: View {
    ...
    @ObservedObject private var viewModel = DiscoverViewModel.shared
    ...
    private let tabDoubleTapPublisher = NotificationCenter.default.publisher(for: .discoverTabDoubleTapped)
    ...
    private func performSearch() {
        ...
        Task {
            do {
                ...
                LoggingService.shared.debug(.books, "Search error: \(error)", component: "DiscoverView")
                unifiedSearchResult = nil
            }
            isSearching = false
        }
    ...
    private func loadPopularSearches() async {
        ...
        do {
            ...
            if let cached: [PopularSearch] = await cacheService.get(cacheKey, type: [PopularSearch].self) {
                ...
                LoggingService.shared.debug(.books, "Failed to load popular searches: \(error)", component: "DiscoverView")
            }
        }
    }
    ...
    private func fetchSuggestions(for query: String) async {
        ...
        do {
            ...
            LoggingService.shared.debug(.books, "Failed to fetch suggestions: \(error)", component: "DiscoverView")
            suggestions = []
        }
    }
    ...
    @ViewBuilder
    private var discoverTabsContent: some View {
        if viewModel.tabs.isEmpty {
            VStack(spacing: 16) {
...
private struct DiscoverTabPage: View {
    let tab: DiscoverTab
    ...
    private func bookListView(
        ...
        ForEach(Array(books.enumerated()), id: \.element.id) { index, bookWithScore in
            DiscoverBookRow(bookWithScore: bookWithScore)
                .padding(.horizontal)
                .padding(.vertical, 8)
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            Image(systemName: isFavorited ? "heart.fill" : "heart")
                ...
                .contentShape(Rectangle())
                .onTapGesture {
                    LoggingService.shared.debug(.books, "❤️ [DiscoverBookRow] Heart tapped! bookId: \(book.id)", component: "DiscoverView")
                    LoggingService.shared.debug(.books, "❤️ [DiscoverBookRow] Current isFavorited: \(isFavorited)", component: "DiscoverView")
                    Task {
                        LoggingService.shared.debug(.books, "❤️ [DiscoverBookRow] Calling toggleFavorite...", component: "DiscoverView")
                        let success = await favoritesManager.toggleFavorite(bookId: book.id)
                        LoggingService.shared.debug(.books, "❤️ [DiscoverBookRow] toggleFavorite returned: \(success)", component: "DiscoverView")
                        if success {
                            isFavorited.toggle()
                            LoggingService.shared.debug(.books, "❤️ [DiscoverBookRow] isFavorited toggled to: \(isFavorited)", component: "DiscoverView")
                        }
                    }
                }
